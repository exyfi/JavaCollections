package queue;
//узнать, какой размер должен быть после clear у массива
//а вообще как лучше делать clear? можно ведь прост size = 0
...
public class ArrayQueueModule {
    ...
    private static void ensureCapacity(int capacity) {
       ...
       /* for (int i = 0; i < size; i++) {
        ...
        System.arraycopy(elements, begin, newElements, 0, size - (size + begin) % elements.length);
        System.arraycopy(elements, 0, newElements, size - (size + begin) % elements.length, (size + begin) % elements.length);
